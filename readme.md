...
- 소켓통신을 통해 서버는 모든 클라이언트와 필요한 정보를 주고받습니다.
- 클라이언트에서 이벤트(건물짓기 등) 처리 시 새로운 스레드를 생성하여 처리합니다.<br/><br/>
...
## 플로우 차트
![KakaoTalk_20201217_083443155](https://user-images.githubusercontent.com/63082842/102422213-96b27480-4049-11eb-94e1-e765c8e77844.jpg)
...
### PLAYERBUILD
** 플레이어가 시티타일의 건물 구매 시 건물을 보여지게하는 기능 **
<ol>
	<li> 플레이어의 발판 이벤트 발동(구매 다이얼로그) 후 구매하기 버튼 클릭 시 발동합니다.
	<li> 플레이어 아이디와 타일번호, 구매한 건물 정보를 서버로 보냅니다.
	<li> 서버는 받은 구매 정보를 모든 클라이언트에게 보냅니다.
	<li> 클라이언트 내부의 건물이 보여지게 하는 함수를 호출합니다.
</ol>

### PLAYERISLAND
** 플레이어가 섬타일의 타일 구매 시 타일의 섬 이름의 색을 변경하는 기능 **